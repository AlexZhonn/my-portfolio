...
export default async function Page({ params }: Props) {
  ...
  return (
    <main className="min-h-screen flex flex-col justify-start pt-20">
      <Link
        href="/"
        ...
        {page.type === "list" && (
          <div className="flex flex-col w-full gap-4">
            {page.items?.map((item: any) => (
              <article
                key={item.id}
                className="group p-6 rounded-lg border border-gray-700 hover:border-gray-500 transition-all duration-300 bg-gray-900/30 hover:bg-gray-900/50"
              >
